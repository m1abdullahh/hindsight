...
import { computeTimeTotals, computeTimeTotalsByDay } from './service.js';
...
export const timeTotalsByDayHandler = async (req: Request, res: Response): Promise<void> => {
  const caller = requireCaller(req);
  const orgId = req.params['orgId'];
  if (!orgId) throw new AppError('invalid_input', 400, 'missing orgId in path');
  const m = caller.membership;
  ...
  const result = await computeTimeTotalsByDay(
    orgId,
    m,
    req.query as unknown as TimeTotalsByDayQuery,
  );
  res.status(200).json(result);
};
