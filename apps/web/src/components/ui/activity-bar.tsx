...
/**
 * Horizontal 24-segment activity strip (Option A style). Each segment is one
 * hour of the day; the integer value chooses an opacity bucket of the accent
 * indigo. `null` = offline (light grey). `'idle'` = amber.
 */
...
export function ActivityBar({
  ...
  return (
    <div
      className={cn('flex flex-1 gap-0.5 rounded-md bg-background p-0.5', className)}
      style={{ height }}
    >
      {segments.map((seg, i) => {
