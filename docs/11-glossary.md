...
**Assignment** — The link between a project and a user, granting that user the ability to track time on the project. Stored in `project_assignments`.
...
**Capture flash** — Optional unobtrusive overlay shown when a screenshot is captured, so the user is reminded recording is happening.
...
**Device token** — A long-lived bearer credential issued by `POST /devices/register` and stored in the OS keychain on the user's machine. Authenticates desktop API calls.
...
**Session** — A web login. Stored in Postgres with a Redis cache. 30-day rolling expiry, cookie-based.
